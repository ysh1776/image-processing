...
int main() {
	...
	out = fopen("2018253096_bmp_output1.bmp", "wb");//쓰기할 파일
	...
	if (data) {
		...
		for (count -= (long)BIH.biWidth; count >= 0; count -= (long)BIH.biWidth)		//raw데이터 값 거꾸로 넣어주기
			fwrite((data + count), 1, (long)((((BIH.biWidth * BIH.biBitCount) + 31) & ~31) >> 3), out);
